<h3>Login with TwoFA via mail using modules</h3>
...
if (
    ...
    $mailaddress = SAMPLE_ACCOUNT_MAIL_ADDRESS; //add code to get assigned mailaddress here;
    $postTwofaSendmailResponse = $twofa->postTwofaSendmail($mailaddress);

    if ($postTwofaSendmailResponse->hasError()) {
        echo 'Error: ' . $postTwofaSendmailResponse->getError();
        exit(0);
    }
    $challengeToken = $postTwofaSendmailResponse->getChallengeToken();
...
?>
    ...
    <form id="code-form" method="post">
        ...
        <br /><label for="code">Code sent to <?php echo $mailaddress; ?></label>
